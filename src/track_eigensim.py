...
def eval_seq(opt, dataloader, data_type, result_filename, save_dir=None, show_image=True, frame_rate=30, use_cuda=True):
    ...
    prev_online_targets = []
    prev_img = None
    eigen_threshold = int(opt.similarity_threshold)
    ...
    for i, (path, img, img0) in enumerate(dataloader):
        ...
        if i > 0 :
          ...
          eig = 1000
        logger.debug('eig_' + str(i) + ": " + str(eig))
        ...
        if eig >= eigen_threshold or num_consecutive_skips >=  detect_frame_interval:
          online_targets = tracker.update(blob, img0)
          prev_online_targets = online_targets
          prev_img = img0
          num_detect+=1
          num_consecutive_skips = 0
          logger.debug('detect at ' + str(i))
        else:
